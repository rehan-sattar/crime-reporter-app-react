...
class Crime_Status extends React.Component {
  constructor() {
    super();
    this.state = {
  ...
  getCrimeData(cityName) {
    if (this.props.reports[cityName]) {
      if (this.props.reports[cityName]['crimes']) {
        let array = [];
        let dataObject = this.props.reports[cityName]['crimes'];
...
export default connect(mapStateToProps)(Crime_Status);
